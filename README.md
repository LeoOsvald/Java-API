...
Professor: Luciano Rodrigo Ferretto Data de Entrega: 22/11/2025
...
Autenticação e Autorização (Auth): Permite que novos usuários se cadastrem (/auth/signup) e que usuários existentes obtenham um token de acesso JWT (/auth/signin) para interagir com as rotas protegidas da API.
Gestão de Usuários: O sistema diferencia usuários comuns (TypeUser.Common) e, implicitamente, suporta a base para futuros papéis de administrador ou organizador de eventos.
Gestão de Eventos: Permite a criação, atualização, listagem e exclusão de eventos. Inclui uma funcionalidade de busca específica por título do evento.
Gestão de Ingressos: Permite o gerenciamento dos tipos de ingressos associados a um evento (e.g., preço, quantidade, tipo).
Gestão de Pedidos: Funcionalidade central para registrar a compra de ingressos, associando-os a um comprador (compradorId) e permitindo a listagem de pedidos por comprador.
...
Autenticação: O endpoint /auth/signin recebe as credenciais do usuário e, se válidas, gera um JWT assinado.
Autorização: O token JWT é enviado em requisições subsequentes no cabeçalho Authorization: Bearer <token>. Um filtro (JwtFilter.java) intercepta essas requisições, valida a assinatura do token e extrai a identidade do usuário, garantindo que apenas usuários autenticados e autorizados acessem os recursos protegidos (Eventos, Ingressos, Pedidos).
